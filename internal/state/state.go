// Package state отвечает за сохранение и восстановление состояния демона
// Позволяет демону помнить был ли он запущен перед отключением
// При перезагрузке системы демон автоматически продолжит работу если был включен
package state
...
// State - структура для сохранения состояния демона на диск
...
type State struct {
	// IsRunning - был ли демон запущен при последнем выключении
	IsRunning bool `json:"is_running"`
}

// Manager - управляет состоянием демона
// Обеспечивает потокобезопасный доступ к состоянию и его сохранение на диск
type Manager struct {
	// filePath - путь где сохраняется состояние (обычно state/daemon.state)
	filePath string
...
func GetInstance() *Manager {
	once.Do(func() {
		// Инициализируем с пустым состоянием (демон не запущен)
		instance = &Manager{
